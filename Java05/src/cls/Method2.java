...
class MyClass5{
	...
	void 메서드1 () {
		// 입력값이 없으면 ()를 비워놓습니다.
		// 반환값이 업으면 자료형 void로 하고  return은 성략
		// return;
		
		// 오로로지 코드 재사용 (입력값x, 반환값x)
		int num = 1;
		System.out.println("나는 메서드"+num+"에 저장된 코드");
	}
		...
		void 메서드2(String str1, String str2) { 
			...
			System.out.println("소괄호로 넘어오는 값만 변경");
			System.out.println("코드재활용"+str2);
		}
		
		// 메서드3: 반환값O, 입력값 (코드 재사요으, 메서드 사용 후에 마지막으로 전달해줄 값이 있음
		String 메서드3() {
			// 메서드의 자료형은 return값이 자료형을 적어준다
			String result = "반갑습니다";
			System.out.println("메서드3을 사용!");
			
			return result;
			// 코드 마지막에 return 옆에이는 값을 전달		
		}
			...
			int 메서드4(double dnum) {
				...
				System.out.println("실수 자료형을 반올립합니다");
